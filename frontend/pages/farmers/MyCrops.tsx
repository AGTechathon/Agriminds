...
import { 
  Plus, Filter, Search, Eye, Edit, Trash2, 
...
export const MyCrop: React.FC = () => {
  console.log("🚀 MyCrop.tsx mounted");
  const { user } = useAuthStore();
  const { crops, setCrops, getCropsByFarmer } = useCropsStore();
  ...
  useEffect(() => {
    api.get('/farmer/crops')
      .then(responseData => {
        ...
        const crops = responseData.map((crop: any) => ({
          ...crop,
      ...
      .catch(err => {
        setCrops([]); // fallback to empty array on error
        console.error('Failed to fetch crops:', err);
      });
  }, [setCrops]);

  const farmerCrops = user ? getCropsByFarmer(String(user.id)) : [];
  console.log("🚀 Farmer's crops:", farmerCrops);
  ...
  const sortedCrops = [...filteredCrops].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'harvest':
        // This one is already correctly handling dates
        return new Date(b.harvestDate).getTime() - new Date(a.harvestDate).getTime();
      case 'newest':
      default:
        // Fix for created_at timestamp
  ...
  const handleDeleteCrop = async (cropId: string) => {
    if (window.confirm('Are you sure you want to delete this crop?')) {
      try {
        await api.delete(`/farmer/crops/${cropId}`);
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t">
                    <div className="flex space-x-2">
                      <Link to={`/farmer/crop/${crop.id}`}>
                        <Button variant="ghost" size="sm" icon={<Eye size={14} />}>
                          View
                        </Button>
                      </Link>
                      {crop.status === 'pending' && (
                        <Link to={`/farmer/edit-crop/${crop.id}`}>
